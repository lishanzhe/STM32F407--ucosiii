...
u8 LCD_Table[LCD_Table_Size] = {0};
	 
//写寄存器函数
//regval:寄存器值
void LCD_WR_REG(vu16 regval)
{
#ifndef BSP_USING_USART_SCREEN
...
void LCD_Clear(u16 color)
{
	...
	u16 i;
	LCD_Table[0] = 0xFF;
	LCD_Table[LCD_Table_Size-1] = 0xFE;
	for (i = 1; i < LCD_Table_Size - 1; ++i)
	{
		LCD_Table[i] = ' ';
	}
#endif
...
#else
	//默认24*12字体
	if (x > 12*(26-1) || y > 36*(13-1))
		return;
	LCD_Table[y/36*26+x/12+1] = num;
#endif
}
//m^n函数
//返回值:m^n次方.
